...
layui.define("layer", function(e) {
	...
	p.prototype.config = {
		...
		var e = this,
			t = e.config,
			n = e.elemFile = i(['<input class="' + u + '" type="file" name="' + t.field + '"', t.multiple ? " multiple" : "",
				">"
			].join("")),
			o = t.elem.next();
		...
		i("#" + f)[0] || i("body").append(n), t.elem.next().hasClass(f) || (e.elemFile.wrap(a), t.elem.next("." + f).append(
			function() {
				var e = [];
			...
			d = {
				select: function() {
							...
							if (!c) {
								...
								f && m.on("keyup", w).on("blur", function(t) {
									...
									var e = t(this),
										a = e.attr("lay-value"),
										n = h.attr("lay-filter");
									return !e.hasClass(u) && (e.hasClass("layui-select-tips") ? m.val("") : (m.val(e.text()), e.addClass(s)),
										e.siblings().removeClass(s), h.val(a).removeClass("layui-form-danger"), layui.event.call(this, l,
											"select(" + n + ")", {
												elem: h[0],
												value: a,
												othis: i
					...
					f.each(function(e, l) {
						...
						var v = "string" == typeof r.attr("lay-search"),
							...
							m = t(['<div class="' + (v ? "" : "layui-unselect ") + a + (c ? " layui-select-disabled" : "") + '">',
								'<div class="' + n + '"><input type="text" placeholder="' + p + '" value="' + (d ? f.html() : "") + '" ' +
								(v ? "" : "readonly") + ' class="layui-input' + (v ? "" : " layui-unselect") + (c ? " " + u : "") + '">',
								'<i class="layui-edge"></i></div>', '<dl class="layui-anim layui-anim-upbit' + (r.find("optgroup")[0] ?
									" layui-select-group" : "") + '">' + function(e) {
									var t = [];
									return layui.each(e, function(e, a) {
										0 !== e || a.value ? "optgroup" === a.tagName.toLowerCase() ? t.push("<dt>" + a.label + "</dt>") : t
											.push('<dd lay-value="' + a.value + '" class="' + (d === a.value ? s : "") + (a.disabled ? " " + u :
				...
				checkbox: function() {
					...
					i.each(function(i, n) {
						...
						var d = l.next("." + c[0]),
							f = t(['<div class="layui-unselect ' + c[0] + (n.checked ? " " + c[1] : "") + (o ?
									" layui-checkbox-disbaled " + u : "") + '" lay-skin="' + (r || "") + '">', {
				...
				radio: function() {
					...
					a.each(function(a, l) {
						...
						if ("string" == typeof r.attr("lay-ignore")) return r.show();
						s[0] && s.remove();
						var c = t(['<div class="layui-unselect ' + e + (l.checked ? " " + e + "ed" : "") + (o ?
								" layui-radio-disbaled " + u : "") + '">', '<i class="layui-anim layui-icon">' + i[l.checked ? 0 : 1] +
							"</i>", "<div>" + function() {
								var e = l.title || "";
...
layui.define(["laytpl", "laypage", "layer", "form"], function(e) {
	...
	S.prototype.config = {
		...
		}), t.height && /^full-\d+$/.test(t.height) && (e.fullHeightGap = t.height.split("-")[1], t.height = T.height() -
			e.fullHeightGap)
		...
		if (t.loading && t.url) return l.msg("数据请求中", {
			icon: 16,
			offset: [e.elem.offset().top + e.elem.height() / 2 - 35 - T.scrollTop() + "px", e.elem.offset().left + e.elem.width() /
				2 - 90 - T.scrollLeft() + "px"
			],
			time: -1,
			anim: -1,
			fixed: !1
		...
		var e, t = this,
			i = t.config,
			a = i.height;
		t.fullHeightGap && (a = T.height() - t.fullHeightGap, a < 135 && (a = 135), t.elem.css("height", a)), e =
